...
constexpr uint8_t epps = ENCODER_PULSES_PER_STEP;
  ...
  char MarlinUI::texto_muy_largo[100];
  ...
  char MarlinUI::capa_actual[20];
  char MarlinUI::total_de_capas[16];

  bool MarlinUI::salida_de_menu = false;

  bool MarlinUI::sensor_filamento_on_off  =false;
  bool MarlinUI::calibracion_on_off       =true;
  bool MarlinUI::informacion_capas_coord  =false;
  ...
  bool MarlinUI::k_melody_on_off          =true;
    ...
    bool MarlinUI::paso_de_varrilla       =false;
  #else
    bool MarlinUI::paso_de_varrilla       =true;
  #endif
  
  float MarlinUI::offset_temporal         =0;

  bool MarlinUI::home_en_accion           =true;

  int MarlinUI::retirar_o_cargar = 0;

  bool MarlinUI::salida_pid = false;

  bool MarlinUI::fan_prueba_pid = false;

  bool MarlinUI::mover_a_punto = false;

  char MarlinUI::list_layer[20][8] = {{""},{""},{""},{""},{""},{""},{""},{""},{""},{""},{""},{""},{""},{""},{""},{""},{""},{""},{""},{""}};

  int MarlinUI::layer_pointer_memory = 0;

  int   MarlinUI::m600_espera_cura = 0;
  bool  MarlinUI::m600_waiting_filament_change = false;  //

  bool MarlinUI::invertir_motor_x = false;

  bool MarlinUI::salida_de_G29 = false;

  bool MarlinUI::salida_de_G26 = false;

  int MarlinUI::modo_de_impresion = 0;

  bool MarlinUI::calibracion_general_valor = false;

  uint8_t MarlinUI::que_eje_hace_home = 0;

  bool MarlinUI::calibrando_sensibilidad = false;

  bool MarlinUI::salida_calibrando_sensibilidad = false;

  bool MarlinUI::sensibilidad_primera_parte = true;

  bool MarlinUI::salida_M117 = false;
  ...
  bool MarlinUI::get_blink() {
    ...
    if (ELAPSED(ms, next_blink_ms)) {
      blink ^= 0xFF;
      next_blink_ms = ms + 800 - (LCD_UPDATE_INTERVAL) / 2;
    }
    return blink != 0;
  }
  ...
  #endif // HAS_LCD_MENU
  #if ENABLED(MENU_KUTTERCRAFT) && ENABLED(PRINTCOUNTER)
    void logros_por_filamento();
    void logros_por_piezas();

    //se encarga de comprobar que el logro alla sido superaso
    void nuevo_logro_pieza(int nivel, float meta_para_logro){
      if(MarlinUI::numero_de_logro_pz == (nivel - 1)){
        printStatistics stats = print_job_timer.getStats();
        //supero el redcord
        if(stats.finishedPrints >= meta_para_logro){
          MarlinUI::mostar_un_solo_logro = 2;
          MarlinUI::goto_screen(logros_por_piezas);

          MarlinUI::numero_de_logro_pz++;
        }
      }
    }

    void nuevo_logro_kilometro(int nivel, float meta_para_logro){
      if(MarlinUI::numero_de_logro_km == (nivel - 1)){
        printStatistics stats = print_job_timer.getStats();
        //supero el redcord
        if(stats.filamentUsed >= meta_para_logro){
          MarlinUI::mostar_un_solo_logro = 1;
          MarlinUI::goto_screen(logros_por_filamento);

          MarlinUI::numero_de_logro_km++;
        }
      }
    }
    //Se encarga de ver si se rompio algun logro de pz
    void hay_un_nuevo_logro_piezas(){
      nuevo_logro_pieza(12, 2015);
      nuevo_logro_pieza(11, 1984);

      nuevo_logro_pieza(10, 1492);
      nuevo_logro_pieza(9,  1024);
      nuevo_logro_pieza(8,  512);
      nuevo_logro_pieza(7,  314);
      nuevo_logro_pieza(6,  138);
      nuevo_logro_pieza(5,  101);
      nuevo_logro_pieza(4,  64);
      nuevo_logro_pieza(3,  42);
      nuevo_logro_pieza(2,  27);
      nuevo_logro_pieza(1,  10);
    }
    //Se encarga de ver si se rompio algun logro de km
    void hay_un_nuevo_logro_filamento(){
      nuevo_logro_kilometro(23, 12756000000);
      nuevo_logro_kilometro(22, 11812000000);
      nuevo_logro_kilometro(21, 5038000000);
      
      nuevo_logro_kilometro(20, 3700000000);
      nuevo_logro_kilometro(19, 2472000000);
      nuevo_logro_kilometro(18, 1605000000);
      nuevo_logro_kilometro(17, 1531000000);
      nuevo_logro_kilometro(16, 1427000000);
      nuevo_logro_kilometro(15, 1105000000);
      nuevo_logro_kilometro(14, 1058000000);
      nuevo_logro_kilometro(13, 914000000);
      nuevo_logro_kilometro(12, 745000000);
      nuevo_logro_kilometro(11, 621000000);

      nuevo_logro_kilometro(10, 505000000);
      nuevo_logro_kilometro(9,  474000000);
      nuevo_logro_kilometro(8,  362000000);
      nuevo_logro_kilometro(7,  311000000);
      nuevo_logro_kilometro(6,  148000000);
      nuevo_logro_kilometro(5,  107000000);
      nuevo_logro_kilometro(4,  53000000);
      nuevo_logro_kilometro(3,  12000000);
      nuevo_logro_kilometro(2,  1000000);
      nuevo_logro_kilometro(1,  100000);
  ...
  void MarlinUI::update() {
    ...
    if (ELAPSED(ms, next_lcd_update_ms) || TERN0(HAS_MARLINUI_U8GLIB, drawing_screen)) {
      ...
      next_lcd_update_ms = ms + LCD_UPDATE_INTERVAL;
        ...
        if(delay_ver_logro <= 100){
          delay_ver_logro++;
        }else{
          hay_un_nuevo_logro_filamento();
          hay_un_nuevo_logro_piezas();
          delay_ver_logro = 0;
        }
        #endif

        //prende el fan del hotend
        if(int(thermalManager.degHotend(0)) >= 50 || MarlinUI::fan_prueba_pid)
          thermalManager.set_fan_speed(2, 255);
        else
          thermalManager.set_fan_speed(2, 0);

        //prende el fan de la electronica
        thermalManager.set_fan_speed(1, 255);
      #endif

        //prende el fan del hotend
        if(int(thermalManager.degHotend(0)) >= 50)
          thermalManager.set_fan_speed(2, 255);
        else
          thermalManager.set_fan_speed(2, 0);
