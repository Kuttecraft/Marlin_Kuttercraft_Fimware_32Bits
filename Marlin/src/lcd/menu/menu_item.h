...
class MenuItem_submenu : public MenuItemBase {
  ...
  class MenuItem_submenu2 : public MenuItemBase {
    public:
      ...
      static inline void action(PGM_P const, const screenFunc_t func) { ui.goto_previous_screen(); ui.goto_screen(func); }
  };
#endif
 ...
 */
  ...
  #define SCREEN_OR_MENU_LOOP(IS_MENU)                    \
      ...
      int32_t _menuLineNr = encoderTopLine, _thisItemNr = 0; \
      bool _skipStatic = IS_MENU; UNUSED(_thisItemNr);      \
      for (int32_t _lcdLineNr = 0; _lcdLineNr < LCD_HEIGHT; _lcdLineNr++, _menuLineNr++) { \
        _thisItemNr = 0
#else
  #define SCREEN_OR_MENU_LOOP(IS_MENU)                    \
    scroll_screen(IS_MENU ? 1 : LCD_HEIGHT, IS_MENU);     \
