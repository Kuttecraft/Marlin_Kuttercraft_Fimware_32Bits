...
#define TMC_EDIT_STORED_I_RMS(ST,STR) EDIT_ITEM_P(uint16_4, PSTR(STR), &stepper##ST.val_mA, 100, 3000, []{ stepper##ST.refresh_stepper_current(); MarlinUI::modo_de_impresion = 3;})
...
void menu_tmc_current() {
  #if ENABLED(MENU_KUTTERCRAFT)
    MarlinUI::ultimo_valor_ventana = menu_tmc_kutter;
  ...
  #define TMC_EDIT_STORED_HYBRID_THRS(ST, STR) EDIT_ITEM_P(uint8, PSTR(STR), &stepper##ST.stored.hybrid_thrs, 0, 255, []{ stepper##ST.refresh_hybrid_thrs(); MarlinUI::modo_de_impresion = 3;});

  void menu_tmc_hybrid_thrs() {
    #if ENABLED(MENU_KUTTERCRAFT)
      MarlinUI::ultimo_valor_ventana = menu_tmc_kutter;
      MarlinUI::ultimo_valor_encoder = 2;
    #endif
    START_MENU();
    BACK_ITEM(MSG_TMC_DRIVERS);
  ...
  #define TMC_EDIT_STEP_MODE(ST, STR) EDIT_ITEM_P(bool, PSTR(STR), &stepper##ST.stored.stealthChop_enabled, []{ stepper##ST.refresh_stepping_mode(); MarlinUI::modo_de_impresion = 3;})
  void menu_tmc_step_mode() {
    #if ENABLED(MENU_KUTTERCRAFT)
      MarlinUI::ultimo_valor_ventana = menu_tmc_kutter;
      MarlinUI::ultimo_valor_encoder = 4;
    #endif
    START_MENU();
