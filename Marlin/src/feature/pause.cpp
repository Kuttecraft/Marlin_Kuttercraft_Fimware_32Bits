...
bool load_filament(const_float_t slow_load_length/*=0*/, const_float_t fast_load_length/*=0*/, const_float_t purge_length/*=0*/, const int8_t max_beep_count/*=0*/,
  ...
  if (fast_load_length) {
    ...
    #if ENABLED(MENU_KUTTERCRAFT)
      float purge_length_ = 0;

      do {
        if(MarlinUI::retirar_o_cargar == 1){
          purge_length_ = 100;
          if (purge_length_ > 0) {
            #if HAS_LCD_MENU
              if (show_lcd) ui.pause_show_message(PAUSE_MESSAGE_CHANGING);
            #endif

            // Extrude filament to get into hotend
            unscaled_e_move(purge_length_, ADVANCED_PAUSE_PURGE_FEEDRATE);
          }
        }else if(MarlinUI::retirar_o_cargar == 2){

          purge_length_ = 30;
          if (purge_length_ > 0) {
            #if HAS_LCD_MENU
              if (show_lcd) ui.pause_show_message(PAUSE_MESSAGE_UNLOAD);
            #endif
          ...
          if (purge_length_ < 0) {
            #if HAS_LCD_MENU
              if (show_lcd) ui.pause_show_message(PAUSE_MESSAGE_INSERT);
            #endif
...
void show_continue_prompt(const bool is_reload) {
  ...
  void wait_for_confirmation(const bool is_reload/*=false*/, const int8_t max_beep_count/*=0*/ DXC_ARGS) {
    ...
    load_filament(slow_load_length, fast_load_length, ADVANCED_PAUSE_PURGE_LENGTH, max_beep_count, true, nozzle_timed_out, PAUSE_MODE_SAME DXC_PASS);
  }
#else
  void wait_for_confirmation(const bool is_reload/*=false*/, const int8_t max_beep_count/*=0*/ DXC_ARGS) {
...
void resume_print(const_float_t slow_load_length/*=0*/, const_float_t fast_load_length/*=0*/, const_float_t purge_length/*=ADVANCED_PAUSE_PURGE_LENGTH*/, const int8_t max_beep_count/*=0*/, const celsius_t targetTemp/*=0*/ DXC_ARGS) {
  ...
  // Retract to prevent oozing
  //unscaled_e_move(-(20), feedRate_t(15));


  #if ENABLED(MENU_KUTTERCRAFT)
    //realiza una extrucion antes de retomar la pieza
    unscaled_e_move(40, feedRate_t(5));
  #endif
  ...
  // Unretract
  #if ENABLED(MENU_KUTTERCRAFT)
    // es menor el numero de rectraccion
    //unscaled_e_move(20, feedRate_t(15));
  #else
    // Unretract
  ...
  #if ENABLED(FWRETRACT) ||  DISABLED(MENU_KUTTERCRAFT)
  ...
  //if (resume_position.e < 0) unscaled_e_move(resume_position.e, feedRate_t(15));
  
  #if ADVANCED_PAUSE_RESUME_PRIME != 0  ||  DISABLED(MENU_KUTTERCRAFT)
