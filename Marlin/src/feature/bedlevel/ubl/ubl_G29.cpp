...
void set_message_with_feedback(FSTR_P const fstr) {
  ...
  void unified_bed_leveling::fine_tune_mesh(const xy_pos_t &pos, const bool do_ubl_mesh_map) {
    ...
    do {
      ...
      #if ENABLED(MENU_KUTTERCRAFT)
        if (!lcd_map_control) ui.goto_screen(calibracion_precisa_espera);// Just editing a single point? Return to status
      #else
      ...
      if (_click_and_hold([]{
        #if ENABLED(MENU_KUTTERCRAFT)
          ui.goto_screen(calibracion_precisa_espera);
        #else
          ui.return_to_status();
        #endif
        do_z_clearance(Z_CLEARANCE_BETWEEN_PROBES);
    ...
    if (lcd_map_control){
      ...
      #if ENABLED(MENU_KUTTERCRAFT)
        mapa_punto_menos_offset();
        ui.goto_screen(menu_kutter_calibracion_con_sensor, 4);
      #else
        ui.return_to_status();
      #endif
    }
