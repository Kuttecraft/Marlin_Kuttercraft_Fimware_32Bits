...
typedef struct SettingsDataStruct {
  ...
  //
    ...
    float _offset_temporal;
    int _encoder_giro;
    int _modo_de_impresion;

    int numero_de_logro_pz_;
    int numero_de_logro_km_;

    char _numero_de_serie[16];
...
#include "../core/debug_out.h"
  ...
  bool MarlinSettings::save() {
    ...
    //
      ...
      EEPROM_WRITE(MarlinUI::sensor_filamento_on_off);
      ...
      EEPROM_WRITE(MarlinUI::offset_temporal);//static float offset_temporal;
      EEPROM_WRITE(MarlinUI::encoder_giro);
      EEPROM_WRITE(MarlinUI::modo_de_impresion);
  ...
  bool MarlinSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      //
        ...
        _FIELD_TEST(MarlinUI::sensor_filamento_on_off);
        EEPROM_READ(MarlinUI::sensor_filamento_on_off);//
        ...
        EEPROM_READ(MarlinUI::k_melody_on_off);

        //offset_temporal
        _FIELD_TEST(MarlinUI::offset_temporal);
        EEPROM_READ(MarlinUI::offset_temporal);

        _FIELD_TEST(MarlinUI::encoder_giro);
        EEPROM_READ(MarlinUI::encoder_giro);

        _FIELD_TEST(MarlinUI::modo_de_impresion);
        EEPROM_READ(MarlinUI::modo_de_impresion); 
...
void MarlinSettings::reset() {
  ...
  //
    ...
    MarlinUI::sensor_filamento_on_off  = false;
    ...
    //dependiendo de la placa cambia el set inicial
    #if(MOTHERBOARD == BOARD_BTT_SKR_V2_0_REV_B)
      MarlinUI::paso_de_varrilla       =false;
    #else
  ...
  void MarlinSettings::report(const bool forReplay) {
    ...
    SERIAL_ECHO_MSG("Hola perra ", OV(25));
    SERIAL_ECHO_MSG("Hola perra ", OVERSAMPLENR);
    SERIAL_ECHO_MSG("Hola perra ", THERMISTOR_TABLE_SCALE);
    SERIAL_ECHO_MSG("Hola perra ", OV_SCALE(25));
  }
