...
#pragma GCC diagnostic pop
  ...
  void Endstops::validate_homing_move() {
      ...
      if (trigger_state()){
        ...
        //se esta calibrando la sensibilidad de los motores
        if(MarlinUI::calibrando_sensibilidad){
          //calibra el eje x
          if(MarlinUI::que_eje_hace_home == 0){
            MarlinUI::salida_calibrando_sensibilidad = true;//avisa que fallo
            hit_on_purpose();//salida

          }else if(MarlinUI::que_eje_hace_home == 1){
            MarlinUI::salida_calibrando_sensibilidad = true;//avisa que fallo
            hit_on_purpose();//salida
          ...
          if(MarlinUI::que_eje_hace_home == 0){
            ...
            SERIAL_ECHOLN("Fallo El Eje Y");
            hit_on_purpose();

          //si el error ocurre en el Z, significa que hay un error real
          //y que puede ser que el sensor inductivo este roto
          //por eso apaga la maquina
          }else{
            kill(GET_TEXT_F(MSG_KILL_HOMING_FAILED));
          }
