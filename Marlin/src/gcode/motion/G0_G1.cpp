...
void GcodeSuite::G0_G1(TERN_(HAS_FAST_MOVES, const bool fast_move/*=false*/)) {
  ...
  #if ENABLED(MENU_KUTTERCRAFT)
    //cambios de filamento programados
    
    //espera a que se ejecuten un par de comando
    //para evitar que se genere un punto
    if(MarlinUI::m600_waiting_filament_change){
      MarlinUI::m600_espera_cura++;
      if(MarlinUI::m600_espera_cura++ > 3){
        MarlinUI::m600_espera_cura = 0;
        MarlinUI::m600_waiting_filament_change = false; //espera a que otro cambio ocurra
        
        //se ejecuta el cambio de filamento 
